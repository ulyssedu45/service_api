...
```js
const { serviceExists, getServiceStatus } = require('service_api');
...
const services = process.platform === 'win32'
  ? ['spooler', 'wuauserv', 'W3SVC']   // Windows
  : ['nginx', 'sshd', 'cron'];          // Linux

const results = await Promise.all(
  services.map(name =>
    getServiceStatus(name).catch(err => ({ name, exists: false, error: err.message }))
  )
);
